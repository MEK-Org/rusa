...
/// against an older server that reports no catalog it degrades to a plain
/// read-only row.
class _VoicePicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (supported.isEmpty) {
      if (actor.voiceName == null) return const SizedBox.shrink();
      return Padding(
        padding: const EdgeInsets.only(top: 16),
        child: Text(
          'Voice: ${actor.voiceName} (instance default)',
          style: const TextStyle(color: MeshColors.textMuted, fontSize: 12),
        ),
      );
    }
