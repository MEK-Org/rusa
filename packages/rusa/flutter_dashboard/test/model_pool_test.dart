...
const _pool = [
  ...
  ProviderModelConfig(provider: 'codex', model: 'gpt-5.6-sol', effort: 'high'),
];
...
void main() {
  group('ThreadDto.modelConfig', () {
    test('parses every candidate in configured order', () {
      ...
      expect(t.modelConfig.map((e) => e.provider), ['claude', 'kimi', 'codex']);
    ...
    test(
      'a malformed candidate fails the parse rather than fabricating one',
      () {
  ...
  testWidgets(
    'one model staged to become a pool reads as a pool on both surfaces',
    (tester) async {
      await tester.runAsync(() async {
        final api = FakeApi()
          ..threadsResult = [
  ...
  testWidgets(
    'a replacement keeping the first candidate still reads as staged',
    (tester) async {
      await tester.runAsync(() async {
        final api = FakeApi()
          ..threadsResult = [
