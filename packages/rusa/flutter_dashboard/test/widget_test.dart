...
void main() {
  ...
  testWidgets('the Info tab retries a failed charter fetch on the next poll', (
    tester,
  ) async {
    await tester.runAsync(() async {
      final api = FakeApi()
      ...
      // The server comes back. The panel is still open on the same actor, so
      // nothing about the selection changes — only the next poll.
      api
        ..charterError = null
        ..charters['a'] = 'Hold the line, and here is the rest of it.';
  ...
  testWidgets('the Info tab picks up a charter edited while it is open', (
    tester,
  ) async {
    await tester.runAsync(() async {
      final api = FakeApi()
      ...
      // The charter is edited. The preview travels with the list, so the poll
      // that carries the new excerpt is what tells the open panel to look again.
      api
        ..threadsResult = [
          makeThread('root', created: 't0'),
          makeThread(
            ...
            parent: 'root',
            created: 't1',
            charterPreview: 'Hold a different line\u2026',
          ),
        ]
        ..charters['a'] = 'Hold a different line, and the rest of that one.';
      await store.refreshThreads();
      for (int i = 0; i < 10; i++) {
        await tester.pump(const Duration(milliseconds: 100));
      }
      ...
      expect(
        find.text('Hold a different line, and the rest of that one.'),
        findsOneWidget,
      );
      expect(
        find.text('Hold the line, and here is the rest of it.'),
