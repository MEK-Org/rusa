...
void main() {
  group('DashboardApi.markInboxHandled', () {
    test('posts the entry id and the operator reason', () async {
      ...
      final api =
          DashboardApi(client: mockClient, base: Uri.parse('http://localhost:3000'));
      await api.markInboxHandled('actor-a', 'entry-1', reason: 'run cancelled by hand');
    ...
    test('throws on a non-200 so the UI can surface the failure', () async {
      final mockClient = MockClient(
        (req) async => http.Response(jsonEncode({'error': 'inbox entry not found'}), 404),
      );
  ...
  group('InboxTab dismiss', () {
    ...
    testWidgets('dismisses an outstanding entry with a typed reason', (tester) async {
      ...
      await tester.enterText(find.byType(TextField), 'prod sent this to staging');
      ...
      expect(api.markInboxHandledCalls.first.reason, 'prod sent this to staging');
    });
    ...
    testWidgets('a failure surfaces instead of looking like it worked', (tester) async {
      api.inboxResultsByStatus['unhandled'] = {
        'entries': [entry('entry-1')]
      };
      api.inboxResultsByStatus['handled'] = {'entries': []};
