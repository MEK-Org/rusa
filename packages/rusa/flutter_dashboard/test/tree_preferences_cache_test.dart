...
void main() {
  group('TreePreferencesCache', () {
    test('NoopTreePreferencesCache never persists — every load returns null', () {
      ...
      cache.saveActorOrder({'root': ['actor-2', 'actor-1']});
      cache.clear();
    ...
    test('FakeTreePreferencesCache tracks saves and loads correctly', () {
      final cache = FakeTreePreferencesCache(
        ...
        storedActorOrder: {'root': ['actor-a', 'actor-b']},
      );

      expect(cache.loadCollapsed(), {'root'});
      expect(cache.loadShowRetired(), isTrue);
      expect(cache.loadActorOrder(), {'root': ['actor-a', 'actor-b']});
      ...
      cache.saveActorOrder({'root': ['actor-b', 'actor-a']});
      expect(cache.saveActorOrderCount, 1);
      expect(cache.loadActorOrder(), {'root': ['actor-b', 'actor-a']});
    ...
    test('DashboardStore seeds collapsed, showRetired, and actorOrder from TreePreferencesCache', () {
      final cache = FakeTreePreferencesCache(
        storedCollapsed: {'actor-a', 'actor-b'},
        storedShowRetired: true,
        storedActorOrder: {'root': ['actor-b', 'actor-a']},
      );
      ...
      expect(store.customActorOrder.value, {'root': ['actor-b', 'actor-a']});
