...
describe("SharedQuotaStore canonical observations", () => {
  ...
  it("removes gpt-reserve from new and legacy Codex projections regardless of scope (#310)", () => {
    const root = mkdtempSync(join(tmpdir(), "rusa-shared-quota-gpt-reserve-"));
    roots.push(root);
    const store = new SharedQuotaStore(join(root, "shared.db"));
    const scrapedAt = "2030-01-01T00:00:00.000Z";
    const resetAtIso = "2030-01-08T00:00:00.000Z";
    const state: ProviderQuotaSnapshot = {
      provider: "codex",
      status: "available",
      scrapedAt,
    ...
    try {
      const id = store.recordRaw({ provider: "codex", scrapedAt, rawOutput: "raw" });
      store.recordParsed(id, state, state);

      // Simulate a row written before the filter existed, including a malformed
      // provider scope. It must be filtered when durable projections are read.
      store.db
        .prepare(
      ...
      const since = "2029-12-31T00:00:00.000Z";
      expect(
        store.listSince("codex", since)[0]?.parsedState?.limits?.map((limit) => limit.label)
      ).toEqual(["Weekly limit"]);
      expect(store.listCanonicalSince("codex", since).map((limit) => limit.label)).toEqual([
        "Weekly limit",
      ]);
      expect(store.listHistorySince("codex", since).map((limit) => limit.label)).toEqual([
        "Weekly limit",
      ]);
    } finally {
      store.close();
