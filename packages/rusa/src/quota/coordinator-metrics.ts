...
/**
 * Build the logger-backed emitter. Records go out at `info`, under the one
 * event name above, so the installed unit runs at the default level and the
 * journal carries lifecycle records and metrics side by side. The event name is
 * the selector — `jq 'select(.msg == "quota_metric")'` isolates the stream and
 * `jq 'select(.msg != "quota_metric")'` removes it — which is what a global
 * `debug` level would have bought at the price of every other component's debug
 * output. At the five-minute service cadence the stream is a few records per
 * provider per tick; the drill transcript reports the observed rate.
 */
export function createQuotaMetrics(logger: Logger): QuotaMetrics {
  const emit = (
    ...
    logger.info(QUOTA_METRIC_EVENT, { metric, type, value, ...labels });
  };
  return {
