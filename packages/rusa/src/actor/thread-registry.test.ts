...
function suite(name: string, make: () => ThreadRegistry) {
  describe(name, () => {
    ...
    it("patches without clobbering other fields", () => {
      ...
      expect(got?.handles).toEqual([{ id: "a", role: "peer" }]);
...
describe("FileThreadRegistry", () => {
  ...
  it("resolves provider aliases and persists the normalized pair on load", () => {
    const file = makeFile();
    writeFileSync(
      file,
      JSON.stringify({
    ...
    const registry = new FileThreadRegistry(file, (provider) =>
