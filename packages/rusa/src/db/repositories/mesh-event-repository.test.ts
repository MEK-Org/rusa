...
describe("MeshEventRepository", () => {
  ...
  describe("listLedgerSourcesAfter", () => {
    it("reads the authoritative body and sender from the staging message spine", () => {
      db.prepare(
        `INSERT INTO mesh_chat (id, ts, sender_id, recipient_id, body, session_id)
         VALUES ('chat-1', '2026-07-21T00:00:00.000Z', 'root', 'actor-a', 'air gap required', NULL)`
      ).run();
      repo.record({
        kind: "message_received",
        actorId: "actor-a",
        payload: JSON.stringify({ messageId: "chat-1", from: "root" }),
      });

      const event = repo.listLedgerSourcesAfter("actor-a", null).events[0];
      expect(event).toMatchObject({ body: "air gap required" });
    });

    it("reads only the target actor's inbound messages forward from an event watermark", () => {
      const first = repo.record({
        kind: "message_received",
        actorId: "actor-a",
        body: "first",
      });
      repo.record({ kind: "run_end", actorId: "actor-a", body: "not a message" });
      repo.record({ kind: "message_received", actorId: "actor-b", body: "other" });
      repo.record({
        kind: "message_received",
        actorId: "actor-a",
        body: "second",
      ...
      expect(repo.listLedgerSourcesAfter("actor-a", null).events.map((e) => e.body)).toEqual([
        "first",
        "second",
      ]);
      expect(repo.listLedgerSourcesAfter("actor-a", first).events.map((e) => e.body)).toEqual([
        "second",
      ]);
    });

    it("interleaves the actor's own yield notes with inbound messages, and nothing else", () => {
      repo.record({ kind: "message_received", actorId: "actor-a", body: "orders" });
      repo.record({ kind: "run_start", actorId: "actor-a", body: "starting" });
      repo.record({
        kind: "run_yielded",
        actorId: "actor-a",
        detail: "blocked",
      ...
      // run_end is the narration stream, not a self-summary — it stays out.
      repo.record({ kind: "run_end", actorId: "actor-a", body: "I will start by listing…" });
      repo.record({ kind: "run_yielded", actorId: "actor-b", body: "another actor's note" });

      const events = repo.listLedgerSourcesAfter("actor-a", null).events;
      expect(events.map((e) => [e.kind, e.body])).toEqual([
        ["message_received", "orders"],
        ["run_yielded", "yield note"],
      ]);
      // A yield note has no peer, and the repository does not invent one — the
      // self-attribution rule lives in the compactor.
      expect(events[1].detail).toBe("blocked");
    });

    it("paginates oldest-first without advancing past unreturned messages", () => {
      repo.record({ kind: "message_received", actorId: "actor-a", body: "one" });
      const second = repo.record({ kind: "message_received", actorId: "actor-a", body: "two" });
      repo.record({ kind: "message_received", actorId: "actor-a", body: "three" });
      const firstPage = repo.listLedgerSourcesAfter("actor-a", null, 2);
      expect(firstPage.events.map((e) => e.body)).toEqual(["one", "two"]);
      expect(firstPage.hasMore).toBe(true);
      expect(repo.listLedgerSourcesAfter("actor-a", second, 2).events.map((e) => e.body)).toEqual([
        "three",
      ]);
    });
  });
