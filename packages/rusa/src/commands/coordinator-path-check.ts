...
import { COORDINATOR_SERVICE_UNITS, POOL_CLIENT_UNITS } from "./coordinator-provisioning.js";
...
export function checkCoordinatorPathDrift(
    ...
    return [
      unknown
        ? {
            ...
            probed: COORDINATOR_SERVICE_UNITS.map((unit) => `${unit}: not established`),
          }
        : {
            name: CHECK_NAME,
            status: "info",
  ...
  return installed.map(({ unit, reading }) => {
    ...
    return compareCoordinator(deps, unit, reading, donor, opts.providerCommands);
  });
}

/** The instance unit sharing this coordinator's basename, e.g. `rusa-staging.service`. */
function pairedInstanceUnit(coordinatorUnit: string): string {
  return coordinatorUnit.replace(/-quota-coordinator\.service$/, ".service");
}
...
export interface CoordinatorPathCheckOptions {
...
/**
 * Clone-local entry point behind `pnpm --filter rusa run check:coordinator-path`.
 *
 * Deliberately not the quickstart preflight suite, which answers "can this
 * checkout run `pnpm start`"; this asks only about the units the host runs.
 */
export function runCoordinatorPathCheck(options: CoordinatorPathCheckOptions = {}): DoctorResult[] {
  ...
  console.log(formatDoctorResults(results, "[rusa] Coordinator PATH check:"));
  return results;
}
