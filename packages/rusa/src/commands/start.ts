...
const FOLLOWER_UPDATE_ACCEPTANCE_TIMEOUT_MS = 5000;
    ...
    updateToolDepsFor = (selfId) => ({
      ...
      deps: {
        ...
        followerCoordinator: {
          updateAll: async (sha) => {
            if (followerHub) {
              const accepted = await followerHub.updateAllFollowersAndWait(
                { targetSha: sha },
                FOLLOWER_UPDATE_ACCEPTANCE_TIMEOUT_MS
              );
              if (!accepted) {
                throw new Error(
                  "One or more followers did not acknowledge their update command before leader restart"
                );
              }
            }
          },
