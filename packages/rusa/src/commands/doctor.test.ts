...
const STAGING_INSTANCE = "rusa-staging.service";
...
function deps(
	...
	return {
		probeSystemdUnitPath: (unit) => ({
			answered: true,
			path: systemdPaths[unit] ?? null,
		}),
		readUnitFile: () => null,
		// Providers live in /opt/providers/bin; everything else resolves nowhere.
		resolveExecutable: (command, pathEnv) =>
			pathEnv.includes("/opt/providers/bin")
				? `/opt/providers/bin/${command}`
				: null,
		...overrides,
	};
}
...
describe("checkCoordinatorPathDrift (#638)", () => {
	...
	it("reports agreement as agreement", () => {
		...
		expect(result.message).toBe(
			`${COORDINATOR} and ${INSTANCE} assign the same PATH.`,
		);
		expect(result.probed).toEqual([
	...
	it("warns without naming a CLI when the provider list could not be determined and the PATHs differ", () => {
		const [result] = checkCoordinatorPathDrift(
			...
			{
				providerCommands: {
					unavailable: "Config file not found at /srv/rusa/config.yaml",
				},
			},
		);
	...
	it("checks the environment-derived coordinator unit too, when that is the one installed", () => {
		...
		expect(results[0].message).toContain(
			`${STAGING_COORDINATOR} PATH has drifted`,
		);
	});

	it("pairs an environment-derived coordinator with its own instance unit on a two-instance host (#643)", () => {
		const STAGING_PATH = "/opt/staging/providers/bin:/usr/bin:/bin";
		const [result] = checkCoordinatorPathDrift(
			deps({
				[STAGING_COORDINATOR]: STAGING_PATH,
				[INSTANCE]: PROVIDER_PATH,
				[STAGING_INSTANCE]: STAGING_PATH,
			}),
			{ providerCommands: { commands: ["codex"] } },
		);
		...
		expect(result.status).toBe("pass");
		expect(result.message).toBe(
			`${STAGING_COORDINATOR} and rusa-staging.service assign the same PATH.`,
		);
	});
	...
	it("warns when the coordinator is installed and assigns no PATH", () => {
		...
		expect(result.status).toBe("warn");
		expect(result.message).toBe(
			`${COORDINATOR} is installed but assigns no PATH.`,
		);
	});

	it("warns when no instance unit assigns a PATH to compare against", () => {
		const [result] = checkCoordinatorPathDrift(
			deps({ [COORDINATOR]: PROVIDER_PATH }),
			{
				providerCommands: { commands: ["codex"] },
			},
		);
...
describe("resolveProviderCommands", () => {
	it("reads the configured provider CLIs from RUSA_HOME when no home is passed", () => {
		...
		expect(resolveProviderCommands()).toEqual({
			commands: ["claude", "codex-cli"],
		});
	});
