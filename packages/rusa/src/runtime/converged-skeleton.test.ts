...
import { type ActorInvocationInputs, RunManager } from "./run-manager.js";
...
describe("Converged Architecture Skeleton", () => {
  ...
  describe("RunManager (Content-free poke, priority, single-flight & invocation inputs)", () => {
    ...
    it("consumes ONLY execution inputs without actor hierarchy and constructs via createActorRuntime", async () => {
      const inboxRepo = new DurableInboxItemRepository();
      ...
      // Duck rootness check: ActorRuntimeProfile does not have a role or isRoot
      const profile = {
        actor: { id: "worker-xyz" },
        capabilities: invocationInputs.capabilities,
        workspace: invocationInputs.workspace,
        driver: invocationInputs.driver,
        options: invocationInputs.options,
        terminal: invocationInputs.terminal,
      };
      // @ts-expect-error role is not part of profile
      expect(profile.role).toBeUndefined();
      // @ts-expect-error isRoot is not part of profile
      expect(profile.isRoot).toBeUndefined();
    });
