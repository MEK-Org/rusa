...
//
// Everything the drill creates lives under a fresh temporary directory: its own
// RUSA_HOME, database, socket, backup directory and workers directory.
//
// The coordinator it starts is a real child process, and what that process can
// reach is constrained rather than described: it is spawned with a constructed
// environment (this process's own env is not inherited) whose PATH contains
// only a directory of stub shims, and whose HOME and XDG_* point inside the
// scratch directory. So no provider CLI runs — `codex`, `claude` and the rest
// resolve to a shim that records the attempt and exits non-zero — and nothing
// resolves the invoking user's provider credentials, because there is no
// `~/.codex` to find. Both drills assert the shim log is empty; a drill that
// touched a real provider account would fail rather than pass quietly.
//
// What is not claimed: the child is an ordinary process, not a sandbox. It
// could still write to an absolute path outside the scratch directory if the
// code under test did that, and the drill would not stop it. The guarantee is
// over what the drill and its configuration point at, plus the two doors —
// PATH and HOME — through which a probe would otherwise reach the real
// provider CLIs and the real credentials.
//
// Usage:
...
class Coordinator {
  ...
  async start() {
    ...
    this.child = spawn(
      ...
      {
        // Constructed, not inherited: see the header. `RUSA_LOG_LEVEL` is
        // deliberately absent — the metric records are emitted at info under
        // their own event name, so the drill reads them without the service
        // running at a level no deployment would.
        env: this.opts.env,
...
/**
 * How many metric records the service actually wrote, and how fast.
 *
 * Reported because the records are emitted at `info` now rather than behind a
 * unit-wide `debug`, which makes their volume an operational question: this is
 * the number an operator can multiply out against their journal retention.
 */
function metricRecordRate(coordinator, elapsedMs, tickSeconds) {
  const records = coordinator.metricRecords();
  const byMetric = new Map();
  for (const record of records) {
    const metric = record.metric ?? "(unnamed)";
    byMetric.set(metric, (byMetric.get(metric) ?? 0) + 1);
  }
  const breakdown = [...byMetric.entries()]
    .sort((a, b) => b[1] - a[1])
    .map(([metric, count]) => `${metric} ${count}`)
    .join(", ");
  const perMinute = elapsedMs > 0 ? (records.length / elapsedMs) * 60_000 : 0;
  return `${records.length} quota_metric records over ${Math.round(elapsedMs / 1000)} s of drill (≈${perMinute.toFixed(1)}/min at a ${tickSeconds}s tick, against the shipped 300s default): ${breakdown}`;
}
...
async function drillServiceDown(ctx) {
  const { t, args, provider, coordinator, readers, databasePath, backupDir, socketPath, stubLog } =
    ctx;
  ...
  t.note(
    `coordinator started; start → /v1/readyz 200: ${boot1} ms (this first boot includes the boot backup: the backup directory is empty, so one is due, and it is taken before readiness is announced)`
  );
  ...
  t.check(
    ...
    lastApplied.every(
      (r, i) => (r.error || r.published === null) && r.applied === lastGood[i].applied
    ) && sinceLastRead < args.hardStaleMs,
    `${sinceLastRead} ms into the outage, still inside hardStaleAfterMs (${args.hardStaleMs} ms): ` +
      lastApplied
        .map((r, i) => `${readers[i].name}: applied ${r.applied}s (${r.error ?? "outage null"})`)
        .join("; ")
  );
  ...
  let after = await Promise.all(readers.map((r) => r.tick(provider)));
  const reconnectDeadline = Date.now() + 5000;
  while (Date.now() < reconnectDeadline && after.some((r) => r.published === null)) {
    await delay(250);
    after = await Promise.all(readers.map((r) => r.tick(provider)));
  }
  ...
  const stubs = stubInvocations(stubLog);
  t.check(
    "no provider CLI was reached: the stub PATH recorded nothing",
    stubs.length === 0,
    stubs.length === 0 ? "stub invocation log empty" : stubs.join("; ")
  );
  t.note(metricRecordRate(coordinator, Date.now() - drillStartedAt, args.tickSeconds));
...
async function drillProbesFailing(ctx) {
  const { t, args, provider, coordinator, readers, home, databasePath, stubLog } = ctx;
  ...
  t.check(
    "the probe failed where it was broken — creating its working directory — and not somewhere else",
    typeof scrape?.error === "string" && scrape.error.includes("ENOTDIR"),
    `error=${scrape?.error ?? "(none)"}`
  );
  ...
  const stubs = stubInvocations(stubLog);
  t.check(
    "no provider CLI was reached: the probe failed before it could spawn one",
    stubs.length === 0,
    stubs.length === 0 ? "stub invocation log empty" : stubs.join("; ")
  );
  t.note(metricRecordRate(coordinator, Date.now() - drillStartedAt, args.tickSeconds));
...
/**
 * Commands a provider probe could plausibly reach for. `bash` and `tmux` are on
 * the list because the codex probe's scrape drives the real CLI through them:
 * shimming the shell is what makes "the probe tried to run something" a
 * recorded, failing event rather than a real `codex` invocation.
 */
const STUBBED_COMMANDS = ["codex", "claude", "agy", "antigravity", "kimi", "bash", "tmux", "bwrap"];

/**
 * Write the stub PATH the coordinator child gets.
 *
 * Config-level stubbing was the obvious route and does not work here: the codex
 * probe calls its scraper without threading `cliCommand` through, and
 * `cliCommand` doubles as the key of the configured-provider set, so pointing it
 * at `/bin/false` would silently reconfigure which providers the drill is about.
 * A PATH the child cannot escape is both simpler and stronger — it covers the
 * commands the scrape reaches for indirectly, which config never could.
 *
 * Each shim appends its argv to `stubLog` and exits non-zero. The drills assert
 * that file is empty; if the probe layer ever does reach a CLI, the drill says
 * so instead of quietly touching a real provider account.
 */
function writeStubBin(binDir, stubLog) {
  ...
  for (const command of STUBBED_COMMANDS) {
    writeFileSync(
      ...
      `${[
        "#!/bin/sh",
        `printf '%s %s\n' "${command}" "$*" >> "$RUSA_DRILL_STUB_LOG"`,
        `echo "${command}: refused by the rollback drill's stub PATH" >&2`,
        "exit 127",
      ].join("\n")}\n`,
      { mode: 0o755 }
    );
...
/** Lines the stub shims recorded, if any. */
function stubInvocations(stubLog) {
...
function setupScratch(t, args, opts) {
  ...
  mkdirSync(join(root, "xdg"), { recursive: true, mode: 0o700 });
  mkdirSync(join(root, "tmp"), { recursive: true });
  writeStubBin(binDir, stubLog);

  // The child's whole environment. Nothing is inherited: PATH resolves only to
  // the stubs, and HOME/XDG_* point at the scratch directory, so a probe that
  // went looking for `~/.codex/auth.json` finds an empty home rather than the
  ...
  return {
    ...
    args,
    provider: args.provider,
    coordinator: new Coordinator({ cli: args.cli, home, socketPath, env }),
